...
const Colors = () => {
  ...
  const [myform, setMyform] = useState({fullname:"",gender:"",email:"",emailconfirm:"",password:"",passwordconfirm:"",mobile:"",affliliation:"",officeph:"",promocode:""});
...
useEffect(() => {
  ...
  const handleChange3 = (value) => {
   ...
   setHargagrandtotal(0);
    ...
    workshopprice.filter(ch => ch.profession === value).map((item) =>
      ...
      // console.log(item.price.replace("IDR ","").replace(",",""))
      );
    // setHargaworkshop
  };
  ...
  const handleChange4 = (value) => {
    ...
    setHargatotal(value.split("#")[1].replace("IDR ","").replace(",","").replace(",",""));
    setHargagrandtotal(value.split("#")[1].replace("IDR ","").replace(",","").replace(",",""));
      ...
      .then((data) => {
        ...
        if (temp.sukses=="OK") {
          if (temp.type=="NUMERIC") {
            ...
            alert("Promo valid diskon "+(temp.nilai).toString());
          } else {
            alert("Promo valid diskon "+(temp.nilai).toString()+"%");

          }
          if (temp.type=="NUMERIC") {
            setHargapromo(temp.nilai);
            setHargagrandtotal(hargagrandtotal-temp.nilai);
          }
          if (temp.type=="PROSEN") {
            setHargapromo(hargagrandtotal*temp.nilai/100);
            setHargagrandtotal(hargagrandtotal*(100-temp.nilai)/100);
          }

          setMyform({...myform,promocode:value})
          setPromofirst(true);
        } else {
...
if (pilih===1 || pilih===3 || pilih===4) simposium2=simposium;
 ...
 axios({
  ...
  url: "https://acsasurabaya2021.com/wp-content/plugins/perki/kirimdata.php",
  // url: "http://localhost/perki/kirimdata.php",
  data: {"firstname":myform.firstname,"lastname":myform.lastname,"profession":professionselect,"fullnamecert":myform.fullname,"gender":myform.gender,"placedob":myform.pob,"address":"","country":myform.county,"province":myform.province,"city":myform.city,"email":myform.email,"password":myform.password,"mobilephone":myform.mobile,"affiliation":myform.affliliation,"officemobilephone":myform.officeph,"simposium":simposium2,"workshopprice":hargaworkshop,"package":packageselect,"workshop":macamworkshop,"payment":"Mid Trans","promocode":myform.promocode,"total":hargagrandtotal,"mtd":'ADDANGGOTA',"idsponsor":idsponsor,"pilih":pilih==1 ? "simposium" : pilih==2 ? "workshop":pilih2},
  contentType: "application/json",
})
  .then((data) => {
...
//        setIsModalVisible(true);
        axios({
          url: "https://acsasurabaya2021.com/wp-content/plugins/perki/kirimdata.php",
          // url: "http://localhost/perki/kirimdata.php",
          data: {mtd: "ANGGOTA3",email:myform.email},
          contentType: "application/json",
          method: 'POST',
        })
          .then((data) => {
    ...
    .then((data) => {
      ...
      if (data.data.sukses==="OK") {
        if (width <= 768) {
          ...
          openInNewTab('https://acsasurabaya2021.com/wp-content/plugins/perki/midtrans/examples/snap/checkout-process2.php?orderid='+orderid+'&desktop=OK&total='+hargagrandtotal+'&judul='+myform.fullname+'-'+professionselect+'-'+myform.mobile);
        }
...
if (step=="Step1")
{
    ...
    if (myform.email!=myform.emailconfirm) {
      alert("email dan confirm tidak sama"); return;
    }
    if (myform.password!=myform.passwordconfirm) {
      alert("password dan confirm tidak sama"); return;
    }
    ...
    axios({
      ...
      .then((data) => {
        ...
        if (temp.sukses=="OKPAY") {
          ...
          return;
        } else  if (temp.sukses=="OK") {
          if (window.confirm("Email sudah ada apa mau ditindih datanya ?")) {
            axios({
              url: "https://acsasurabaya2021.com/wp-content/plugins/perki/kirimdata.php",
              // url: "http://localhost/perki/kirimdata.php",
              data: {mtd:"HAPUSEMAIL",email:myform.email},
              contentType: "application/json",
              method: 'POST',
            })
              .then((data) => {
                ...
                if (temp.sukses=="OK") {
                  ...
                  alert("Penghapusan data tidak sukses");return;
                }
...
<CRow>
      {/* <div class="labeldaninput" style={{marginLeft:"15px"}}>
          <div class="labelatas">First Name</div>
          <Input placeholder="First Name" value={myform.firstname} onChange={(e)=>setMyform({...myform,firstname:e.target.value})} />
      </div>
      <div class="labeldaninput" style={{marginLeft:"15px"}}>
          <div class="labelatas">Last Name</div>
          <Input placeholder="Last Name"  value={myform.lastname} onChange={(e)=>setMyform({...myform,lastname:e.target.value})} />
      </div> */}
      <div class="labeldaninput" style={{marginLeft:"15px"}}>
      ...
      {/* <div class="labeldaninput" style={{marginLeft:"15px"}}>
          <div class="labelatas">Place of Birth</div>
          <Input placeholder="Place of birth"  value={myform.pob} onChange={(e)=>setMyform({...myform,pob:e.target.value})} />
      </div>
      <div class="labeldaninput" style={{marginLeft:"15px"}}>
          <div class="labelatas">Date of Birth</div>
          <DatePicker style={{width:"100%"}} onChange={(date, dateString)=>setMyform({...myform,dob:dateString})}/>
      </div>
      <div class="labeldaninput" style={{marginLeft:"15px"}}>
          <div class="labelatas">Country</div>
          <Select defaultValue="Indonesia" style={{ width: "100%" }} >
          <Option value="Indonesia">Indonesia</Option>
          </Select>
      </div>
      <div class="labeldaninput" style={{marginLeft:"15px"}}>
          <div class="labelatas">Propinsi</div>
          <Select id="propinsi" name="propinsi" onChange={(e)=>handleChange(e)} value={myform.province} onChange={(e)=>setMyform({...myform,province:e.target.value})}  style={{ width: "100%" }} >
...
<Option value="Papua ( Daerah Khusus )">Papua ( Daerah Khusus )</Option>
      ...
      <div class="labeldaninput" style={{marginLeft:"15px"}}>
          <div class="labelatas">City</div>
          <Select id="kota" name="kota" value={kotaselect} onChange={(e)=>handleChange2(e)} value={myform.city} onChange={(e)=>setMyform({...myform,city:e.target.value})}   style={{ width: "100%" }} >
    ...
    <Select style={{width:"100%"}} onChange={(e)=>handleChange5(e)} >
     {workshopprof.filter(ch => ch.profession === professionselect).map((item) =>(
      ...
      <div class="labeldaninput">
          ...
          <Select  onChange={(e)=>handleChange4(e)} style={{width:"100%"}} value={packageselect}>
          {hargapaket.filter(character => character.profession === professionselect).map((item) =>(

<Option value={item.package+"#"+item.price+"#"+item.jumlah}>{item.package}</Option>
...
<div class="labeldaninput">
      ...
      {/* <div class="labeldaninput">
          ...
          <Select defaultValue="Mid Trans" style={{width:"100%"}}>
      ...
      <Button type="primary" onClick={()=>lastaction()}>Bayar</Button>
      </div>
</CRow>
</CCardBody>
</CCard>
...
<Modal title="Pembayaran Midtrans" visible={isModalVisible} onOk={handleOk} >
  {/* Total : {myform.total} <Button onClick={()=>openInNewTab('https://acsasurabaya2021.com/wp-content/plugins/perki/midtrans/examples/snap/checkout-process.php?orderid='+record.orderid+'&total='+record.total+'&judul='+record.firstname+'-'+record.profession+'-'+record.pilih+'-'+record.mobilephone)}>Pay</Button> */}
      </Modal>
</>
  )
}
